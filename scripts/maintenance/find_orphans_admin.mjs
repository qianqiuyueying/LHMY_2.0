...
const pagesDir = path.join(adminSrc, 'pages')
const routerFile = path.join(adminSrc, 'router', 'index.ts')
...
function relFromPages(fullPath) {
...
function main() {
  ...
  for (const p of pages) {
    const rel = relFromPages(p)
    if (!used.has(rel)) orphans.push(rel)
  }

  console.log(`admin pages total: ${pages.length}`)
  console.log(`admin pages referenced by router: ${used.size}`)
  if (orphans.length === 0) {
