...
- [x] 发送短信验证码：`POST /api/v1/auth/request-sms-code`（scene=`H5_BUY`）  
  - 证据：`frontend/h5/src/pages/BuyPage.vue` `sendSms()`
- [x] 登录：`POST /api/v1/auth/login`（channel=`H5`）  
  - 证据：`frontend/h5/src/pages/BuyPage.vue` `ensureLogin()`
- [x] 创建订单：`POST /api/v1/orders`（Header: `Idempotency-Key`；Query: `dealerLinkId` 必填）  
...
- [x] **步骤 3：后端登录/下单/发起支付**
  - 短信：`POST /api/v1/auth/request-sms-code`（scene=`H5_BUY`）
  - 登录：`POST /api/v1/auth/login`（channel=`H5`）
  - 下单：`POST /api/v1/orders`（Header: `Idempotency-Key`；Query: `dealerLinkId`）
