...
def _qr_payload_v1(*, entitlement_id: str, voucher_code: str) -> str:
    ...
    return build_payload_text(entitlement_id=entitlement_id, voucher_code=voucher_code, ts=now_ts, nonce=nonce, sign=sign)
...
"""
    ...
    if body.redemptionMethod not in {RedemptionMethod.QR_CODE.value, RedemptionMethod.VOUCHER_CODE.value}:
    ...
    async with session_factory() as session:
        ...
        if e.valid_from and e.valid_from > now:
        ...
        if vs.redemption_method != body.redemptionMethod:
            raise HTTPException(status_code=409, detail={"code": "REDEEM_NOT_ALLOWED", "message": "核销方式不匹配"})

        # 预约前置（属性16）
        booking = None
        if not can_redeem_with_booking_requirement(booking_required=bool(vs.booking_required), has_confirmed_booking=False):
        ...
        if body.redemptionMethod == RedemptionMethod.QR_CODE.value:
            ...
            if not vr.ok:
                raise HTTPException(status_code=403, detail={"code": vr.error_code or "QR_SIGN_INVALID", "message": "二维码签名无效"})
        ...
        new_state = apply_redeem(entitlement_type=e.entitlement_type, remaining_count=int(e.remaining_count), success=True)
...
async def list_entitlements(
    ...
    # USER/ADMIN 皆可；USER 仅本人 ownerId
    admin_ctx = await _try_get_admin_context(authorization)
    ...
    return ok(
        data={"items": [_entitlement_dto(x) for x in rows], "page": page, "pageSize": page_size, "total": total},
        request_id=request.state.request_id,
    )
...
async def get_entitlement_detail(request: Request, id: str, authorization: str | None = Header(default=None)):
    ...
    return ok(data=_entitlement_dto(e), request_id=request.state.request_id)
...
"""
    ...
    actor_type = "ADMIN" if admin_ctx else "USER"
    actor_id = str(admin_ctx["adminId"]) if admin_ctx else str(user_ctx["userId"])
    ...
    async with session_factory() as session:
        ...
        if e.entitlement_type == EntitlementType.SERVICE_PACKAGE.value and e.service_package_instance_id:
            sp = (
                await session.scalars(
                    select(ServicePackageInstance).where(ServicePackageInstance.id == e.service_package_instance_id).limit(1)
                )
            ).first()
            if sp is None:
            ...
            if any(x.remaining_count != x.total_count for x in entitlements):
                raise HTTPException(status_code=409, detail={"code": "STATE_CONFLICT", "message": "服务包已使用，不可转赠"})
            ...
            if redeemed_count > 0:
                raise HTTPException(status_code=409, detail={"code": "STATE_CONFLICT", "message": "服务包已核销，不可转赠"})
            ...
            if e.remaining_count != e.total_count:
                raise HTTPException(status_code=409, detail={"code": "STATE_CONFLICT", "message": "权益已使用，不可转赠"})
            ...
            if redeemed_count > 0:
                raise HTTPException(status_code=409, detail={"code": "STATE_CONFLICT", "message": "权益已核销，不可转赠"})
